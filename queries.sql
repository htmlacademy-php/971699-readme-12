...
SELECT  p.id, p.creation_date, p.title, u.login, p_t.title, p.count_views FROM posts p JOIN users u ON p.author_id = u.id JOIN post_types p_t ON p.post_type_id = p_t.id ORDER BY count_views DESC;
...
SELECT c.content, u.login FROM comments c JOIN posts p ON c.post_id = p.id JOIN users u ON c.author_id = u.id WHERE p.id = 2;
